...
**Resolved (2026-08-20, BC-QA-031):** All monetary amounts are displayed in EUR only. The clash was first closed by dual-currency (BGN+EUR) display gated by a `currency_transition_window_open` flag (`utils/currencyDisplay.ts` and all call sites across admin/partner/user routes and services); Bulgaria's BGN→EUR transition has since completed and that machinery has been removed in full, leaving no transition-window flag, module, or wrapper object. Answering the questions above: amounts — including historical transactions and past reporting periods — stay BGN-denominated in storage and are converted to EUR at the fixed currency-board rate before display, so no record carries a flip date and no report is rendered in BGN. The requirement is stated in `06-admin-spec-extracted.md` §3.7, `07-partner-spec-extracted.md` §7.3 and `08-user-spec-extracted.md` §17; the decision record is `04-clash-resolution-analysis.md` Clash 12.1.

...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { email, password, firstName, lastName, phone, acceptTerms, acceptPrivacy, accountType, businessInfo } = req.body;
 ...
 *   - User.passwordHash is set to the supplied password
 *
 * The `password` field is REQUIRED. Partners never choose a password at
 * registration (RegisterPartnerPage collects none, and admin-onboarded
 * partners only have a random temp password they were never shown) — every
 * partner sets their password here at activation. The frontend
 * PartnerActivatePage always sends one.
 *
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Password is REQUIRED — every partner sets their password at activation
    // (none is collected at registration). Reject missing/empty up front, then
    // enforce the canonical password policy (8+ chars with complexity) so
    // partner activation cannot set a weaker password than /auth/register.
    if (!password || typeof password !== 'string') {
      return res.status(400).json({ success: false, error: 'password is required' });
    }
    // NB: request-shape validation (password policy) runs before the DB token
    // lookup by design — fail fast on malformed input without a DB round-trip,
    // the conventional order. A bogus token still fails inside
    // consumeActivationToken below; only the *error precedence* differs (password
    // format reported before token validity), which is acceptable and avoids a
    ...
    try {
      ...
      detach(fireAutomation('partner.approved', {
        partnerId: result.partnerId,
        recipientName: result.businessName,
      }), (err2) => logger.error('[automation] partner.approved fire failed (activate):', err2));
 ...
 * Update user profile
 * Spec §5.1: PARTNER role cannot directly edit profile fields — changes must go
 * through the Help (change-request) flow. Block PARTNER role here.
 */
router.put(
  '/profile',
  authenticate,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    // Spec §5.1 partner-role guard
    if (req.user!.role === 'PARTNER') {
      return res.status(403).json({
        success: false,
        error: 'Partners cannot directly edit profile fields. Please submit a change request via the Help system.',
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (req.user!.role === 'PARTNER') {
      return res.status(403).json({
        success: false,
        error: 'Partners cannot change email via the self-service flow. Please submit a change request via the Help system.',
        code: 'PARTNER_USE_CHANGE_REQUEST',
      });
    }
    const userId = req.user!.id;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Align to the canonical password policy (8+ chars with complexity) so the
 ...
 * Spec §5.4 / §10.7: PARTNER account closure requires a 30-day notice period
 * submitted via the Help system — self-service deletion is blocked.
 */
router.delete(
  '/account',
  authenticate,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    // Spec §5.4 / §10.7 partner-role guard
    if (req.user!.role === 'PARTNER') {
      return res.status(403).json({
        success: false,
        error: 'Partner account closure requires a 30-day notice period. Please submit an account closure request via the Help system.',
        code: 'PARTNER_USE_HELP_FOR_CLOSURE',
      });
    }
...
// Canonical password policy as a Zod rule: 8+ chars with upper/lower/digit/special.
// Mirrors validatePasswordPolicy() so complete-profile sets passwords under the
// exact same rule as /auth/register, change-password, activate and reset-password.
const completeProfilePassword = z
  .string()
...
const completeProfileSchema = z.object({
  ...
  password: completeProfilePassword,

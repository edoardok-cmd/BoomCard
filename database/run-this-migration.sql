...
CREATE INDEX idx_business_cards_user ON public.business_cards(user_id);
...
('user', 'Regular User', '["read:own_profile", "update:own_profile", "create:cards", "read:own_cards", "update:own_cards", "delete:own_cards"]'),
('premium', 'Premium User', '["read:own_profile", "update:own_profile", "create:cards", "read:own_cards", "update:own_cards", "delete:own_cards", "analytics:detailed", "export:data"]')
...
CREATE POLICY "Users can read own profile" ON auth_service.users
    FOR SELECT USING (auth.uid() = id);

CREATE POLICY "Users can update own profile" ON auth_service.users
    FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Users can read own cards" ON public.business_cards
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create own cards" ON public.business_cards
...
DO $$
BEGIN
    RAISE NOTICE 'BoomCard database setup completed successfully!';
END $$;